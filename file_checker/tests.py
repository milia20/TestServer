...
from .models import UploadedFile, MyUser
...
class UploaderTests(APITestCase):
    ...
    @classmethod
    def setUp(cls):
        u = MyUser.objects.create_user('test_user', password='test', email='test@mail.ru')
        u.save()
        t = Token.objects.create(user=u)
        t.save()
        UploadedFile.objects.create(owner=u, file=os.path.basename(open(r'media\covid_worldwide.csv').name)).save()
        UploadedFile.objects.create(owner=u, file=os.path.basename(open(r'media\Survey_AI.csv').name)).save()

    def test_unauthorized_upload(self):
        url = reverse('file_checker:upload')
        client = APIClient()
        f = open(r'test_datasets\prc_hpi_a__custom_3617733_page_linear.csv')
        data = {'datafile': f}
    ...
    def test_authorized_upload(self):
        ...
        response = self.client.post(url, data, format='multipart', REMOTE_USER='test_user')
    ...
    def test_authorized_drop(self, url, status, count):
        ...
        self.assertEqual(response.status_code, status)
        self.assertEqual(UploadedFile.objects.count(), count)